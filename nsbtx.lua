local function readDict(file, readFunc, args)
    local log = io.open("log_dict.txt", "w")
    file:seek("cur", 1)
    local size = string.unpack("<b", file:read(1))
    log:write("Reported Dict Size is ", size, "\n")
...
local function readPalette(file, args)
    ...
    local log = io.open("log_read_pal.txt", "w")

    local colorCount = args[2] / 2
    log:write("# of Colors ")
    log:write(colorCount)
...
local function import(plugin)
    ...
    dlg:button {
        ...
        onclick = function()
            ...
            local log = io.open("log_main.txt", "w")
